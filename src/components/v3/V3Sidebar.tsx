      ...
      <nav className="flex-1 overflow-y-auto py-3 v3-scrollbar">
        {NAV_SECTIONS.map(section => (
          <div key={section.label} className="mb-4">
            {!collapsed && (
              <div
                className="px-4 mb-1 text-[10px] font-semibold tracking-widest"
        ...
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <div className="text-xs font-medium truncate" style={{ color: 'var(--v3-text)' }}>Officer Yılmaz</div>
              <div className="text-[10px]" style={{ color: 'var(--v3-text-muted)' }}>Analyst</div>
            </div>
          )}
          {!collapsed && (
            <button
