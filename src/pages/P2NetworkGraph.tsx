...
import {
  ...
  ZoomIn, ZoomOut, Maximize2, RotateCcw, Eye, EyeOff, X,
  Shield, ExternalLink, ChevronRight, Crosshair, Layers,
...
const NODE_HEX: Record<NodeType, number> = {
  applicant:      0x0a1628,
  person:         0x6b7280,
  flagged_person: 0xef4444,
  organization:   0x3b82f6,
  social:         0x8b5cf6,
  location:       0x10b981,
};
...
const EDGE_COLORS: Record<EdgeType, string> = {
  KNOWS:        'rgba(156,163,175,0.4)',
  CONNECTED_TO: 'rgba(59,130,246,0.5)',
  FOLLOWS:      'rgba(139,92,246,0.4)',
  EMPLOYED_BY:  'rgba(245,158,11,0.5)',
...
const NODES: GNode[] = [
  { id: 'ahmad', label: 'Ahmad Rezaee', type: 'applicant', riskScore: 94, detail: 'Primary subject — Iranian national, Visa Mill network member. Score: 94/100.', flagged: true },
...
function buildNodeGeometry(type: NodeType, size: number): THREE.BufferGeometry {
  switch (type) {
    ...
    case 'flagged_person':
      return new THREE.OctahedronGeometry(size, 0); // sharp diamond
    case 'organization':
...
// ── Component ──
export default function P2NetworkGraph() {
  ...
  const riskNodeIds = useMemo(() => {
    ...
    // include direct neighbors of flagged
  ...
  const linkColor = useCallback((link: GLink) => {
    ...
    return EDGE_COLORS[link.edgeType] || 'rgba(255,255,255,0.15)';
  }, []);
  ...
  return (
    <div className="relative w-full h-[calc(100vh-64px)] bg-[#080c14] overflow-hidden" ref={containerRef}>
      {/* 3D Graph */}
      <ForceGraph3D
        ref={graphRef}
        graphData={graphData}
      ...
      <div className="absolute top-4 left-4 right-4 flex items-center justify-between pointer-events-none z-10">
        ...
        <div className="pointer-events-auto flex items-center gap-2">
          <div className="bg-[#0f1524]/90 backdrop-blur-md border border-white/10 rounded-lg px-2">
        ...
        <div className="bg-[#0f1524]/90 backdrop-blur-md border border-white/10 rounded-xl p-3 space-y-3 w-48">
            ...
            return (
              <button key={t} onClick={() => toggleType(t)}
                className={cn('flex items-center gap-2 w-full text-left text-xs rounded-md px-2 py-1.5 transition-all',
                  active ? 'text-white/90 bg-white/5' : 'text-white/30 hover:text-white/50'
                )}>
                <Icon size={12} />
                <span className="flex-1">{label}</span>
                <span className="text-[10px] text-white/30">{count}</span>
                <div className={cn('w-1.5 h-1.5 rounded-full', active ? 'bg-blue-400' : 'bg-white/10')} />
              </button>
            );
          })}
          ...
          <div className="border-t border-white/5 pt-2 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-white/40">Particles</span>
              <Switch checked={showParticles} onCheckedChange={setShowParticles} className="scale-75" />
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-white/40">Risk paths</span>
              <Switch checked={showRiskPaths} onCheckedChange={setShowRiskPaths} className="scale-75" />
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-white/40">Risk glow</span>
      ...
      <div className="absolute right-4 top-20 pointer-events-auto z-10 flex flex-col gap-1.5">
        {[
          ...
          <Button key={tip} size="icon" variant="ghost" onClick={action} title={tip}
            className="w-9 h-9 bg-[#0f1524]/90 backdrop-blur-md border border-white/10 text-white/60 hover:text-white hover:bg-white/10">
            <Icon size={15} />
          </Button>
        ))}
      </div>
        ...
        <div className="bg-[#0f1524]/90 backdrop-blur-md border border-white/10 rounded-xl px-4 py-2 flex items-center gap-5">
          {[
            ...
            <div key={label} className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] text-white/40 uppercase tracking-wide">{label.replace('_', ' ')}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="absolute bottom-4 right-4 pointer-events-auto z-10">
        <div className="bg-[#0f1524]/90 backdrop-blur-md border border-white/10 rounded-xl px-3 py-2 flex items-center gap-4">
          <div className="text-center">
            ...
            <div className="text-[9px] text-white/30">Flagged</div>
          </div>
          <div className="w-px h-6 bg-white/10" />
          <div className="text-center">
            <div className="text-sm font-bold text-orange-400">{NODES.filter(n => n.riskScore >= 60).length}</div>
            <div className="text-[9px] text-white/30">High Risk</div>
          </div>
          <div className="w-px h-6 bg-white/10" />
          <div className="text-center">
            <div className="text-sm font-bold text-blue-400">{LINKS.length}</div>
            <div className="text-[9px] text-white/30">Links</div>
          </div>
        </div>
      </div>
          ...
          >
            <div className="p-5 space-y-5">
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    {(() => { const Icon = TYPE_ICONS[dossierNode.type].icon; return <Icon size={14} className="text-white/50" />; })()}
                    <span className="text-[10px] uppercase tracking-wider text-white/40">{TYPE_ICONS[dossierNode.type].label}</span>
                  </div>
                  <h3 className="text-base font-bold text-white">{dossierNode.label}</h3>
                </div>
              ...
              <div className="flex items-center gap-3">
                <div className={cn('text-2xl font-bold', riskColor(dossierNode.riskScore))}>
                  {dossierNode.riskScore}
                </div>
                <div>
                  <div className="text-[10px] text-white/40 uppercase tracking-wide">Risk Score</div>
                ...
                {dossierNode.flagged && (
                  <Badge className="bg-red-500/20 text-red-400 border border-red-500/30 text-[9px] px-1.5">
              ...
              {/* Detail */}
              <p className="text-xs text-white/60 leading-relaxed">{dossierNode.detail}</p>
              ...
              {(dossierNode.riskScore >= 60 || dossierNode.flagged) && (
                <div className="bg-red-500/5 border border-red-500/10 rounded-lg p-3 space-y-1.5">
                  ...
                  {dossierNode.id === 'ahmad' && (
                    <>
                      <p className="text-[11px] text-white/50">• Visa Mill network membership confirmed</p>
                      <p className="text-[11px] text-white/50">• Document hash match (Maris: DOC-IR-4892)</p>
                      <p className="text-[11px] text-white/50">• Duplicate mobile correlation (vizesepetim.com)</p>
                      <p className="text-[11px] text-white/50">• 3+ flagged associates in network</p>
                    </>
                  )}
                  {dossierNode.id === 'reza' && (
                    <>
                      <p className="text-[11px] text-white/50">• EU Sanctions familial match</p>
                      <p className="text-[11px] text-white/50">• Shared document hash with Ahmad</p>
                      <p className="text-[11px] text-white/50">• Active in flagged Telegram groups</p>
                    </>
                  )}
                  {dossierNode.id !== 'ahmad' && dossierNode.id !== 'reza' && (
                    <p className="text-[11px] text-white/50">• Elevated risk score based on network analysis</p>
                  )}
                </div>
              )}
              ...
              {/* Connections */}
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-wider text-white/40 mb-2 flex items-center gap-1.5">
                ...
                <div className="space-y-1">
                  {dossierConnections.map((c, i) => (
                    ...
                    >
                      ...
                      <span className="text-[11px] text-white/70 flex-1 truncate">{c.other?.label ?? '?'}</span>
                      <span className={cn('text-[9px] px-1.5 py-0.5 rounded border', riskBg(c.other?.riskScore ?? 0))}>
                        {c.other?.riskScore ?? 0}
                      </span>
              ...
              <div className="space-y-2 pt-2 border-t border-white/5">
                <Button variant="outline" size="sm" className="w-full bg-white/5 border-white/10 text-white/70 hover:bg-white/10 text-xs justify-start gap-2">
                  <Eye size={12} /> View Full Dossier
                </Button>
                <Button variant="outline" size="sm" className="w-full bg-white/5 border-white/10 text-white/70 hover:bg-white/10 text-xs justify-start gap-2">
                  <ExternalLink size={12} /> Open in Case Detail
                </Button>
                <Button variant="outline" size="sm" className="w-full bg-red-500/10 border-red-500/20 text-red-400 hover:bg-red-500/20 text-xs justify-start gap-2">
          ...
          >
            <div className="bg-[#0f1524]/90 backdrop-blur-md border border-white/10 rounded-xl p-3 w-64">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className={cn('text-xs font-bold', riskColor(selectedNode.riskScore))}>{selectedNode.riskScore}</span>
              </div>
              <p className="text-[10px] text-white/40 mb-2">{selectedNode.detail}</p>
              <Button size="sm" variant="ghost" onClick={() => setDossierNode(selectedNode)}
                className="w-full text-[10px] text-blue-400 hover:text-blue-300 hover:bg-blue-500/10 h-7">
