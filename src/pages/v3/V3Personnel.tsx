...
import { defenceScans, type PersonnelResult, type RiskLevel } from '@/data/v3/mockData';
import { RiskBadge } from '@/components/v3/V3Badges';

// Flatten all personnel from all scans
const allPersonnel = defenceScans.flatMap(scan =>
  scan.results.map(p => ({ ...p, scanId: scan.scanId, scanDate: scan.startedAt }))
);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 p-3 border rounded-md" style={{ background: 'var(--v3-surface)', borderColor: 'var(--v3-border)' }}>
        ...
        <input
          type="text" placeholder="Search personnel..." value={search}
        ...
        <table className="w-full text-xs">
          <thead>
            <tr style={{ borderBottom: '1px solid var(--v3-border)' }}>
              {['Name', 'Rank', 'Unit', 'Branch', 'Profiles Found', 'Violations', 'OPSEC Risk', 'Scan ID'].map(h => (
          ...
          <tbody>
            {filtered.map(p => (
              <tr key={`${p.scanId}-${p.id}`} style={{ borderBottom: '1px solid var(--v3-border)' }} className="transition-colors hover:bg-white/[0.03]">
                ...
                <td className="px-4 py-2.5 font-mono" style={{ color: 'var(--v3-text)' }}>{p.profilesFound}</td>
                <td className="px-4 py-2.5 font-mono" style={{ color: p.opsecViolations.length > 0 ? 'var(--v3-red)' : 'var(--v3-green)' }}>
                  {p.opsecViolations.length}
                </td>
                <td className="px-4 py-2.5"><RiskBadge level={p.overallRisk} /></td>
                <td className="px-4 py-2.5 font-mono" style={{ color: 'var(--v3-text-muted)' }}>{p.scanId}</td>
              </tr>
            ))}
          </tbody>
        </table>
