...
export default function V3Login() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center v3-grid-bg" style={{ background: 'var(--v3-bg)' }}>
      <div className="w-full max-w-sm">
        <form onSubmit={handleSubmit} className="border rounded-md p-8" style={{ background: 'var(--v3-surface)', borderColor: 'var(--v3-border)' }}>
          <div className="text-center mb-8">
            <h1 className="text-lg font-bold tracking-[0.3em]" style={{ color: 'var(--v3-text)' }}>ALPAGU</h1>
          ...
          <div className="mb-4">
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)}
              ...
              placeholder="operator@alpagu.gov.tr" required />
          </div>

          <div className="mb-6">
            <label className="block text-[11px] font-semibold mb-1.5 tracking-wide" style={{ color: 'var(--v3-text-secondary)' }}>PASSWORD</label>
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--v3-text-muted)' }}>
          ...
          <div className="mt-4 text-center">
            <button type="button" onClick={fillDemo} className="text-xs transition-colors hover:underline" style={{ color: 'var(--v3-text-muted)' }}>
              Use demo credentials
            </button>
          </div>
        </form>
        <p className="text-center mt-4 text-[10px]" style={{ color: 'var(--v3-text-muted)' }}>Classified system. Unauthorized access is a criminal offense.</p>
      </div>
    </div>
  );
}
